...
| `Shift+Alt+↑` / `Shift+Alt+↓` | Insert cursor above/below |
...
\* `Alt+Click` may not work on some Linux distros — change the modifier via the `editor.multiCursorModifier` setting (e.g. to `Ctrl+Click`).
...
| `Ctrl+Shift+C` | Open new command prompt/terminal (OS-level) |
...
| `Ctrl+Shift+↑` / `Ctrl+Shift+↓` | Scroll up/down |
